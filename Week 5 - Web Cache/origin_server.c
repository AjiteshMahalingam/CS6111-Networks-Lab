...
int main(){
    ...
    printf("[+] Server set-up and running at port %d\n", PORT);
    ...
    while(1){
        ...
        if(clientSocket < 0){
            printf("[-] Proxy server accept request failed.\n");
            exit(1);
        }
        printf("[+] Proxy server connected.\n");
        ...
        if(read(clientSocket, &request, sizeof(request)) > 0){
            ...
            printf("[+] Proxy server requested data for reg.no %ld - Semester %d.\n",rollNum, sem);
            ...
            struct Student student = students[sem-1];
            response.responseBody = students[sem-1];
            ...
            printf("[+] Proxy server request for reg.no %ld - Semester %d completed and connection closed.\n", student.regNum, student.sem);
            close(clientSocket);
        }
    }
    close(serverSocket);
