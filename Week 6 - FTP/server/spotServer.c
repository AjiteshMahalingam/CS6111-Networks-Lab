...
int sendFile(int dataSocket, char fname[], int start){
    printf("\nSending file - %s - from %d bytes.\n", fname, start);
    FILE *fp = fopen(fname, "r");
    fseek(fp, start, SEEK_SET);
    int cnt = 0;
    if(fp){
        char c;
        while(cnt < MAX_FILE_SIZE){
            c = fgetc(fp);
            //printf("%c", c);
        ...
        fclose(fp);
        if(c != EOF){
            sendFile(dataSocket, fname, start+MAX_FILE_SIZE);
        }        
        return 0;
    } else {
        return -1;
...
void retreiveHandler(int clientSocket, struct FTP_Command command, int start){
    int dataSocket = establishDataConn();
    if(dataSocket < 0){
        return;
    }
    ...
    if(strncmp("users.txt", fname, len) == 0){
        ...
        if(flag){
            ...
            if(sendFile(dataSocket, fname, start) >= 0){                
                reply.reply = 226;
...
void storeHandler(int clientSocket, struct FTP_Command command){
    int dataSocket = establishDataConn();
    if(dataSocket < 0){
        return;
    }
...
void listHandler(int clientSocket, struct FTP_Command command){
    int dataSocket = establishDataConn();
    if(dataSocket < 0){
        return;
    }
            ...
            while(1){
                ...
                if(strncmp(command.command, "QUIT", 4) == 0){
                    ...
                    if(strncmp(command.command, "RETR", 4) == 0)
                        retreiveHandler(clientSocket, command, 0);
                    else if(strncmp(command.command, "STOR", 4) == 0)
                        storeHandler(clientSocket, command);
                    else if(strncmp(command.command, "LIST", 4) == 0)
                        listHandler(clientSocket, command);
