...
void initDS();
int clientSocket;
void *sendResponse(void *reqStudent){
    struct Student student = *((struct Student *)reqStudent);
    ...
    printf("[+] Client request for reg.no %ld - Semester %d completed and connection kept alive.\n", student.regNum, student.sem);
    ...
    while(1){
        ...
        while(1){
            ...
            if(read(clientSocket, &request, sizeof(request)) > 0){
                ...
                pthread_create(&requestThreads[sem-1], NULL, sendResponse, (void *)&reqStudent);
            }
        }
        for(int i = 0; i < 4; i++)
