...
void recvFile(int dataSocket, int serverSocket, char fname[], int start){
    ...
    if(c != EOF){
        recvFile(dataSocket,serverSocket, fname, start + MAX_FILE_SIZE);
    }
}
void retrHandler(int controlSocket, int dataSocket, struct FTP_Command command){
    ...
    if(strncmp("users.txt", fname, len) == 0){
        read(controlSocket, &reply, sizeof(reply)); // 553 Permission denied    
        printf("S > %d  %s\n",reply.reply, reply.msg); 
    } else {
        FILE *fp = fopen(fname, "w");
        fclose(fp);
        recvFile(dataSocket, serverSocket, fname, 0);    
        read(controlSocket, &reply, sizeof(reply)); // 226 Transfer Complete.
        printf("S > %d  %s\n",reply.reply, reply.msg);
